...
when defined(chronosStrictException):
  template createCb(paramType, iteratorNameSym,
    ...
    identName = proc(udata: pointer) {.gcsafe, raises: [Defect].} =
      ...
      try:
        ...
        if not(nameIterVar.finished()):
          var next = nameIterVar(fut)
          # Continue while the yielded future is already finished.
          while (not next.isNil()) and next.finished():
            next = nameIterVar(fut)
          ...
          if next == nil:
            ...
            next.addCallback(identName, udata)
...
proc asyncSingleProc(prc: NimNode): NimNode {.compileTime.} =
  ...
  if procBody.kind != nnkEmpty:
    ...
    var procCb = getAst createCb(internalFutureType, iteratorNameSym,
    ...
    # -> iterator(cast[pointer](resultFuture))
    outerProcBody.add(
        newCall(cbName,
          newNimNode(nnkCast)
          .add(newIdentNode("pointer"))
          .add(retFutureSym)
        )
    )
